			...
			while(enumeration.hasMoreElements()){
				...
				if(tmp.length() > 6 && tmp.substring(tmp.length()-6).compareTo(".class") == 0) {
					...
					for(int i = 0 ; i < tmpClass.getInterfaces().length; i ++ ){
						...
						// trouvée
							if( tmpClass.getInterfaces()[i].getName().toString().equals("com.t3.metamediamanager.Provider") ) {
								this.classProvider.add(tmpClass);
							}
							if( tmpClass.getInterfaces()[i].getName().toString().equals("com.t3.metamediamanager.Saver") ) {
